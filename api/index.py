...
BOT_API_SECRET = os.environ.get('BOT_API_SECRET', 'xK9mP2qR8vL5nJ3w').strip()
...
def auth_bot():
    if BOT_API_SECRET: return hmac.compare_digest(request.headers.get('x-bot-secret',''), BOT_API_SECRET)
    return auth()
...
def create_key():
    ...
    k=newkey(); created=now_ms(); rec={'app_id':app_id,'created_at':created,'expires_at':created+days*86400000,'max_devices':limit,'revoked':False,'devices':{},'owner_telegram_id':str(d.get('owner_telegram_id','') or '')}
    db.reference(f'licenses/{kid(k)}').set(rec)
    return jsonify(ok=True,license_key=k,**rec),201

@app.post('/user/create-key')
def user_create_key():
    if not auth_bot(): return jsonify(ok=False,error='unauthorized'),401
    d=request.get_json(silent=True) or {}
    try:
        days = int(d.get('days', 30))
        limit = int(d.get('max_devices', 1))
        ...
        return jsonify(ok=False, error='days and max_devices must be integers'), 400
    app_id = str(d.get('app_id', '') or '').strip()
    owner_id = str(d.get('owner_telegram_id', '') or '').strip()
    if days < 1 or limit < 0: return jsonify(ok=False, error='invalid parameters'), 400
    if not owner_id: return jsonify(ok=False, error='owner_telegram_id required'), 400
    k=newkey(); created=now_ms(); rec={'app_id':app_id,'created_at':created,'expires_at':created+days*86400000,'max_devices':limit,'revoked':False,'devices':{},'owner_telegram_id':owner_id}
    db.reference(f'licenses/{kid(k)}').set(rec)
    return jsonify(ok=True,license_key=k,**rec),201
...
def user_licenses():
    ...
    result=[dict(v or {},id=k) for k,v in data.items() if isinstance(v,dict) and str(v.get('owner_telegram_id',''))==owner_id]
